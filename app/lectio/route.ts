...
export async function GET() {
  ...
  #nav-botoes {
    display: flex;
    justify-content: flex-start; /* Alinhado à esquerda para o botão "anterior" */
    margin-bottom: 1rem;
  }
  .social-links {
    margin-top: 2rem;
  ...
  .banner-container img {
    ...
    const compartilharBtn = document.querySelector("button[onclick=\"compartilhar()\"]");    function updateUI() {
      ...
      try {
        ...
        if (postsSummary.length > 0) {
          ...
          if (!error) {
            // Apenas define se nenhum erro específico foi capturado
    ...
    function compartilhar() {
      ...
      const texto = "Evangelho do Dia: " + titulo + "\n\n" + conteudo
