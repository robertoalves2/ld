...
<button onclick="navegar(-1)" id="proximo">Próximo dia &rarr;</button>
    ...
    function compartilhar() {
      ...
      const elements = tempDiv.querySelectorAll("h2, h3, p, strong");
      elements.forEach(element => {
        let textContent = element.textContent?.trim() || "";

        // Remove verse numbers for sharing, or keep them as plain numbers
        ...
        if (element.tagName === 'H3') {
          ...
          } else if (/^Meditar a Palavra/i.test(textContent)) {
            shareableText += "🧠 Meditar a Palavra\\n\\n";
          } else if (/^Rezar a Palavra/i.test(textContent)) {
            shareableText += "🙏 Rezar a Palavra\\n\\n";
          } else if (/^Compromisso/i.test(textContent)) {
            shareableText += "📌 Compromisso\\n\\n";
          ...
          clonedElement.querySelectorAll("strong").forEach((strongEl) => {
            const originalStrongText = strongEl.textContent || ""
            strongEl.replaceWith(\`**\${originalStrongText}**\`)
          })
      ...
      if (window.isSecureContext && navigator.canShare && navigator.canShare({ text: finalShareText })) {
        navigator
          .share({
            title: titulo,
            text: finalShareText,
