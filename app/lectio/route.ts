...
export async function GET() {
  ...
  .banner-container img {
    ...
    try {
      const feedRes = await Promise.race([ // Usando const e Promise.race para timeout
        fetch(PROXY_URL + encodeURIComponent(FEED_URL)),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout fetching feed summary')), 15000)) // 15 segundos de timeout
      ]);
      ...
      error = "Erro ao carregar a lista de posts. Tente novamente mais tarde. Detalhes: " + (err instanceof Error ? err.message : String(err));
    } finally {
      loadingSummary = false;
      updateUI();
      if (postsSummary.length > 0) {
        ...
        if (!error) { // Apenas define se nenhum erro específico foi capturado
          error = "Nenhum post encontrado ou carregado.";
        }
        updateUI(); // Chama updateUI novamente para garantir que o estado final seja refletido
      }
    }
  }
  async function fetchAndDisplayPostContent(post) {
    ...
    try {
      ...
      const res = await Promise.race([ // Adicionado Promise.race para timeout
        fetch(proxiedUrl),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout fetching post content')), 15000)) // 15 segundos de timeout
      ]);
