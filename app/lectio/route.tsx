...
function updateUI() {
  ...
  if (loadingSummary || loadingContent) {
    ...
    evangelhoEl.innerHTML = \`<p style="color: red;">\${errorMessage}</p>\`;
...
async function fetchPostsSummary() {
  ...
  try {
    ...
    errorMessage = "Erro ao carregar a lista de posts. Tente novamente mais tarde.";
  } finally {
    loadingSummary = false;
    updateUI();
    if (postsSummary.length > 0) {
...
async function fetchAndDisplayPostContent(post) {
  ...
  try {
    ...
    const article =
      postDoc.querySelector("article") || postDoc.querySelector(".post-content") || postDoc.querySelector("main");
    let content = "Conteúdo não encontrado.";

    if (article) {
      // Remove elementos desnecessários
      const elementsToRemove = article.querySelectorAll("script, style, .social-share, .advertisement, .ads");
      elementsToRemove.forEach((el) => el.remove());

      // Busca por todos os elementos de texto
      const allElements = [...article.querySelectorAll("h1, h2, h3, h4, h5, h6, p, strong, em, blockquote, ul, ol, li")];

      // Procura pelo início do evangelho
      let startIndex = -1;
      for (let i = 0; i < allElements.length; i++) {
        const text = allElements[i].textContent?.trim().toLowerCase() || "";
        if (
          text.includes("evangelho") &&
          (text.includes("segundo") ||
            text.includes("mateus") ||
      ...
      // Se não encontrou pelo evangelho, procura por "lectio divina" ou similar
      if (startIndex === -1) {
        for (let i = 0; i < allElements.length; i++) {
          const text = allElements[i].textContent?.trim().toLowerCase() || "";
          if (
            text.includes("lectio") ||
            text.includes("compreender") ||
            text.includes("meditar") ||
            text.includes("rezar")
          ) {
            startIndex = i;
            break;
      ...
      if (startIndex !== -1) {
        ...
        for (let i = startIndex; i < allElements.length; i++) {
          const elemento = allElements[i];
          const texto = elemento.textContent?.trim() || "";

          if (!texto || textoUsado.has(texto)) continue;

          // Para quando encontrar elementos de fim de conteúdo
          if (/share|partilhar|compartilhar|tags|categorias|autor:/i.test(texto)) break;

          // Adiciona o elemento
          recorteHTML += elemento.outerHTML;
          textoUsado.add(texto);
        }

        // Aplica formatação especial para seções da Lectio Divina
        content = recorteHTML
          .replace(/(compreender\\s+a\\s+palavra)/gi, "<h3>💡 Compreender a Palavra</h3>")
          .replace(/(meditar\\s+a\\s+palavra)/gi, "<h3>🧠 Meditar a Palavra</h3>")
          .replace(/(rezar\\s+a\\s+palavra)/gi, "<h3>🙏 Rezar a Palavra</h3>")
          .replace(/(compromisso\\s+semanal)/gi, "<h3>📌 Compromisso Semanal</h3>")
          .replace(/(compromisso)(?!\\s+semanal)/gi, "<h3>📌 Compromisso</h3>");
      } else {
        // Se não encontrou seções específicas, pega todo o conteúdo do artigo
    ...
    errorMessage = "Erro ao carregar o conteúdo do post.";
    currentPostContent = null;
  } finally {
    loadingContent = false;
    updateUI();
...
🌐 Acesse: https://liturgiadiaria.top/lectio\`;
